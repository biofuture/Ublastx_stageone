mothertable <- read.table(file="testoutdir//stage2output.normalize_16s.mergesubtype.tab.txt", sep="\t",header=T,row.names=1,quote = "", stringsAsFactors = FALSE)
#remove zero line
...
pdf("testoutdir//stage2output.16S.pdf")
...
dev.off()
mothertable <- read.table(file="testoutdir//stage2output.normalize_cellnumber.mergesubtype.tab.txt", sep="\t",header=T,row.names=1,quote = "", stringsAsFactors = FALSE)
#remove zero line
mothertable <- mothertable[which(rowSums(mothertable)!=0),]
mothertable <- t(mothertable)
mothertable <- mothertable[which(rowSums(mothertable)!=0),]
dim(mothertable)
pdf("testoutdir//stage2output.cell.pdf")
vd <- vegdist(mothertable,method="bray")
vd.pco <- pco(vd, k=10)
pcoadata <- data.frame(vd.pco$points[,1], vd.pco$points[,2], cols)
EnvironmentType <- pcoadata$cols
